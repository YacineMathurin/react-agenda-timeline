...
function getLongerMeetingEndTime(index, res) {
  return Math.max(sortedMeetings[index]["end"], res[res.length - 1]["end"]);
}

function getLowestMeetingEndTime(index, res) {
...
function isTimeBetween(index, res) {
...
function handleNotCollapsing(index, res) {
...
function handleCollapsingWithSmallerInterval(index, res) {
  collapsingWithSmallerIntervalCount += 1;
  console.log("handleCollapsingWithSmallerInterval");

  lastElementCollapsed = true;
  lowestMeetingEndTime = getLowestMeetingEndTime(index, res);
  longerMeetingEndTime = getLongerMeetingEndTime(index, res);
...
function handleCollapsingWithBiggerInterval(index, res) {
...
function checker(index, res) {
  if (isTimeBetween(index, res) && !isCollapsingWithLongerInterval(index)) {
...
export function drawTimeLine() {
  ...
  for (let index = 0; index < sortedMeetings.length; index++) {
    ...
    switch (checker(index, res)) {
      case "not_collapsing":
        handleNotCollapsing(index, res);
        break;

      case "collapsing_with_bigger_interval":
        handleCollapsingWithBiggerInterval(index, res);
        break;

      case "collapsing_with_smaller_interval":
        handleCollapsingWithSmallerInterval(index, res);
        break;
    }
  }
  return res;
